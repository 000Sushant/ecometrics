...
@Component({
  ...
  template: `
    <article
      class="impact-card glass-panel"
      [ngClass]="getCardClass()"
      [attr.aria-label]="'Impact analysis for ' + report.articleTitle"
    >
      ...
      <div class="metrics-list-new">
        ...
        <div class="metric-row-new">
          ...
          <div class="metric-text-new">
            <strong>Water:</strong> Melts enough ice to fill {{ getWaterPools() }} Olympic swimming
            pools.
          </div>
        </div>
        ...
        <div class="metric-row-new">
          ...
          <div class="metric-text-new">
            <strong>Land:</strong> Effectively erases the cooling power of
            {{ getLandParks() }} local city parks.
          </div>
        </div>
        ...
        <div class="metric-row-new">
          ...
          <div class="metric-text-new">
            <strong>Habit:</strong> Pollution of driving a car to work for
            {{ getHabitYears() }} years.
          </div>
        </div>
      </div>
      ...
      <footer class="card-footer">
        ...
        <div class="protest-container" (click)="$event.stopPropagation()">
          <button
            class="protest-btn"
            [class.active]="hasProtested"
            (click)="onVote(hasProtested ? 'down' : 'up')"
            aria-label="Protest this carbon impact"
            title="Raise Alarm / Protest this action"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
    ...
    `
      ...
      .impact-card:hover {
        ...
        box-shadow:
          ...
          0 0 1px 1px var(--border-glass) inset;
      }
...
export class ImpactCardComponent {
  ...
  getVotes(): number {
    return (this.report as ImpactReport & { votes?: number }).votes ?? 0;
  }

  getAirPeople(): string {
    const people = Math.round(this.report.co2EquivalentKg / PERSON_OXYGEN_KG_CO2_PER_YEAR);
    return people.toLocaleString();
  }

  getWaterPools(): string {
    const pools = (this.report.co2EquivalentKg * GLACIER_MELT_LITRES_PER_KG) / OLYMPIC_POOL_LITRES;
    return pools.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 2 });
  }

  getLandParks(): string {
    const parks = this.report.co2EquivalentKg / CITY_PARK_KG_CO2;
    return parks.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 });
  }

  getHabitYears(): string {
    const years = Math.round(this.report.co2EquivalentKg / COMMUTE_YEAR_KG_CO2);
